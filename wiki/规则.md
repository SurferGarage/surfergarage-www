...
| `href` | 视频区表现 | 说明 |
|--------|------------|------|
| `http(s)://…` | 显示外链卡 | `isSocialChannelLive(href)` |
| `#` 或非 http | **视频区不渲染** | X / YouTube 占位留在数据里，上线改 href 即可 |
| `#founders` 等站内 hash | 文章区微信卡仍显示 | 仅 **video** 块 `.filter(isSocialChannelLive)` |

Discord：**不在** `social-channels`；入口在 `#call` / `site-contact.ts`。

### 7.2 统计数字

`lib/site-stats.ts` **运行时**从 `WECHAT_OFFICIAL_FEED`、`SURFING_FOUNDERS_SEASON_01` 聚合 → Hero ticker / Proof 等共用，**不要手填重复数字**。

### 7.3 片场四屏

顺序真值：`lib/founder-panels.ts` → 访谈 / 视频 / Playbook / 视觉车库。
...
- 部分微信 `href` 仍指向公众号首页 → 上线前换单篇 URL（`wechat-official-feed.ts`）。
- X / YouTube 数据保留、`href: "#"` → 接好链接后自动出现在视频区。
