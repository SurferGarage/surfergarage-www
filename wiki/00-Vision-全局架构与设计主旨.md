...
- **目标**：浪前片场、社交矩阵、转化等 **可扫读信息**。
- **动效**：随滚动 **scrub** 压低首屏 `--wave-distortion` / `--wave-opacity`（ScrollTrigger 触发区为 **`#founders`**，与主列「宣言 → 片场」衔接）。
...
4. **[编排]** `home-scroll-choreography.tsx`：首屏 → **`#founders`** 浪面收束 scrub、各区块 pin、`ScrollTrigger.refresh` 策略。  
